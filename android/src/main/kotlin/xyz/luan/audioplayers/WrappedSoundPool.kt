...
class WrappedSoundPool internal constructor(
    ...
    companion object {
         ...
         * it has been loaded.
         */
        private val soundIdToPlayer =
            Collections.synchronizedMap(mutableMapOf<Int, WrappedSoundPool>())
         ...
         */
        private val urlToPlayers =
            Collections.synchronizedMap(mutableMapOf<String, MutableList<WrappedSoundPool>>())
            ...
            soundPool.setOnLoadCompleteListener { _, sampleId, _ ->
                ...
                if (loadingPlayer != null) {
                    ...
                    synchronized(urlToPlayers) {
                        val urlPlayers =
                            urlToPlayers[loadingPlayer.url] ?: listOf<WrappedSoundPool>()
    ...
    override fun setUrl(context: Context, url: String, isLocal: Boolean) {
        ...
        synchronized(urlToPlayers) {
            ...
            if (originalPlayer != null) {
                ...
                loading = true
                soundId = soundPool.load(getAudioPath(context, url, isLocal), 1)
    ...
    private fun getAudioPath(context: Context, url: String?, isLocal: Boolean): String? {
        if (isLocal) {
            return url?.removePrefix("file://")
        }
        val proxy = ref.getProxy(context)
